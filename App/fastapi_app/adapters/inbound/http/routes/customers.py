...
from models import Account, Wallet, Movement, CustomerSummary  # Pydantic
...
def get_accounts(customer_id: int, db: Session = Depends(get_db), current_user: UserDB = Depends(get_current_user)):
...
def get_wallet(customer_id: int, db: Session = Depends(get_db), current_user: UserDB = Depends(get_current_user)):
    svcs = build_services(db)
...
def get_summary(customer_id: int, db: Session = Depends(get_db), current_user: UserDB = Depends(get_current_user)):
    svcs = build_services(db)
...
def get_movements(
    ...
    svcs = build_services(db)
    rows = svcs["movements"].execute(customer_id, account_type, date_from, date_to)
    return rows  # Movement ya tiene from_attributes=True en tu models.py :contentReference[oaicite:2]{index=2}
