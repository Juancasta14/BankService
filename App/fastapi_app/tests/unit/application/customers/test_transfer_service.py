...
def test_transfer_success(transfer_service: TransferService, uow: MockUnitOfWork):
    result = transfer_service.transfer(from_account_id=1, to_account_id=2, amount=150.0)
...
def test_transfer_insufficient_funds(
    transfer_service: TransferService, uow: MockUnitOfWork
):
    with pytest.raises(ValueError) as excinfo:
        transfer_service.transfer(
            from_account_id=1, to_account_id=2, amount=600.0  # Acc 1 only has 500
        )
...
def test_transfer_negative_amount(
    ...
    with pytest.raises(ValueError) as excinfo:
        transfer_service.transfer(from_account_id=1, to_account_id=2, amount=-50.0)
...
def test_transfer_same_account(transfer_service: TransferService, uow: MockUnitOfWork):
    with pytest.raises(ValueError) as excinfo:
        transfer_service.transfer(from_account_id=1, to_account_id=1, amount=100.0)
...
def test_transfer_account_not_found(
    ...
    with pytest.raises(ValueError) as excinfo:
        transfer_service.transfer(
            from_account_id=99, to_account_id=2, amount=100.0  # Does not exist
        )
    ...
    with pytest.raises(ValueError) as excinfo_in:
        transfer_service.transfer(
            from_account_id=1, to_account_id=99, amount=100.0  # Does not exist
        )
